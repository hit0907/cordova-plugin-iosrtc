...
function getDisplayMedia(constraints) {
	...
	if (constraints.video === true) {
		...
		if (constraints.video.width) {
			if (typeof constraints.video.width === 'number') {
				...
				if (constraints.video.width.min && typeof constraints.video.width.min === 'number') {
					newConstraints.video.mandatory.minWidth = constraints.video.width.min;
				}
				if (constraints.video.width.max && typeof constraints.video.width.max === 'number') {
		...
		if (constraints.video.height) {
			if (typeof constraints.video.height === 'number') {
				...
				if (constraints.video.height.min && typeof constraints.video.height.min === 'number') {
					newConstraints.video.mandatory.minHeight = constraints.video.height.min;
				}
				if (constraints.video.height.max && typeof constraints.video.height.max === 'number') {
		...
		if (constraints.video.frameRate) {
			if (typeof constraints.video.frameRate === 'number') {
				...
				if (constraints.video.frameRate.min && typeof constraints.video.frameRate.min === 'number') {
					newConstraints.video.mandatory.minFrameRate = constraints.video.frameRate.min;
				}
				if (constraints.video.frameRate.max && typeof constraints.video.frameRate.max === 'number') {
	...
	return new Promise(function (resolve, reject) {
		function onResultOK(data) {
			...
			var stream = new MediaStream(data.stream);
			resolve(stream);
		}
